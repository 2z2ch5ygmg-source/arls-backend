...
AZ_KAKAO_REST_API_KEY="${AZ_KAKAO_REST_API_KEY:-${KAKAO_REST_API_KEY:-}}"
AZ_KAKAO_JAVASCRIPT_KEY="${AZ_KAKAO_JAVASCRIPT_KEY:-${KAKAO_JAVASCRIPT_KEY:-}}"
...
deploy_backend() {
  ...
  az_exec webapp config appsettings set --name "$AZ_BACKEND_APP" --resource-group "$AZ_RG" --output none --settings \
    ...
    KAKAO_REST_API_KEY="$AZ_KAKAO_REST_API_KEY" \
...
deploy_frontend() {
  ...
  if command -v python3 >/dev/null 2>&1; then
    python3 - "$frontend_source/config.js" "$backend_url" "$(date -u +%s)" "$AZ_GOOGLE_MAPS_API_KEY" "$AZ_KAKAO_JAVASCRIPT_KEY" <<'PY'
import pathlib
import sys

path, backend_url, build_id, google_maps_api_key, kakao_javascript_key = sys.argv[1:]
p = pathlib.Path(path)
...
updated_kakao_javascript_key = False
...
for line in lines:
    ...
    if line.startswith("window.ENV_GOOGLE_MAPS_API_KEY"):
        ...
        continue
    if line.startswith("window.ENV_KAKAO_JAVASCRIPT_KEY"):
        result.append(f"window.ENV_KAKAO_JAVASCRIPT_KEY = '{kakao_javascript_key}';")
        updated_kakao_javascript_key = True
        continue
    result.append(line)
...
if not updated_kakao_javascript_key:
    result.insert(3, f"window.ENV_KAKAO_JAVASCRIPT_KEY = '{kakao_javascript_key}';")
...
window.ENV_KAKAO_JAVASCRIPT_KEY = '${AZ_KAKAO_JAVASCRIPT_KEY}';
EOF
  fi
