...
| VIS-024 | OPEN | P0 | Schedule upload missing night support required row | user report 2026-04-19 | Schedule Upload Business Semantics | Night support blocks with no required-count row and no support-worker/external-person payload are blocked as `필요 인원 수를 해석할 수 없습니다.` | Treat missing required-count row with no filled support payload as no request and skip it. | provided workbook parser baseline, unit test, live upload screenshot |
| VIS-025 | OPEN | P0 | Schedule upload inferred night support request count | user report 2026-04-19 | Schedule Upload Business Semantics | Night support blocks with filled support-worker names but no required-count row are blocked even though the worker count is enough to infer request size. | Infer request count from filled support-worker rows when required-count row is absent and all worker values are valid. | provided workbook parser baseline, unit test, live API preview |
